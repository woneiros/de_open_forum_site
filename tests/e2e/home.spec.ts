...
test.describe('Homepage', () => {
  ...
  test('should have links to past events', async ({ page }) => {
    ...
    const talks2024Link = page.getByRole('link', { name: '[talks]' }).first();
    await expect(talks2024Link).toBeVisible();
    await expect(talks2024Link).toHaveAttribute('href', '/past/2024');

    // Check 2025 edition links - get the second [talks] link
    const talks2025Link = page.getByRole('link', { name: '[talks]' }).nth(1);
  ...
  test('should have external links with correct attributes', async ({ page }) => {
    ...
    // Check YouTube playlist links
    const youtubeLinks = page.getByRole('link', { name: /YouTube playlist/i });
    const first2024YoutubeLink = youtubeLinks.first();
    await expect(first2024YoutubeLink).toHaveAttribute('target', '_blank');
    await expect(first2024YoutubeLink).toHaveAttribute('rel', 'noopener noreferrer');

    // Check Google Group link
    const googleGroupLink = page.getByRole('link', { name: 'data-engineering-open-forum' });
    await expect(googleGroupLink).toHaveAttribute('href', 'https://groups.google.com/g/data-engineering-open-forum');
  ...
  test('should display key features and benefits', async ({ page }) => {
    ...
    await expect(page.getByText(/Technically deep, globally relevant/i)).toBeVisible();
    await expect(page.getByText(/Community-driven content/i)).toBeVisible();
    await expect(page.getByText(/Open dialogue & collaboration/i)).toBeVisible();
  });
});
