...
test.describe('2024 Talks Page', () => {
  test('should display event metadata', async ({ page }) => {
    ...
    // Check title
    await expect(page.getByText('Data Engineering Open Forum 2024')).toBeVisible();

    // Check description
    await expect(page.getByText(/inaugural Data Engineering Open Forum/i)).toBeVisible();
    ...
    // Check talk count
    await expect(page.getByText('// 7 talks')).toBeVisible();
  });
  ...
  test('should display playlist link', async ({ page }) => {
    ...
    // Check playlist link
    const playlistLink = page.getByRole('link', { name: /VIEW FULL PLAYLIST ON YOUTUBE/i });
    await expect(playlistLink).toBeVisible();
    await expect(playlistLink).toHaveAttribute('target', '_blank');
    await expect(playlistLink).toHaveAttribute('href', /youtube\.com.*playlist/i);
  });
  ...
  test('should display talk cards with YouTube links', async ({ page }) => {
    ...
    await expect(page.getByText(/Evolving from Rule-based Classifier/i)).toBeVisible();
    await expect(page.getByText(/Machine Learning Powered Auto Remediation/i)).toBeVisible();
  ...
  test('should display author names and titles', async ({ page }) => {
    ...
    await expect(page.getByText(/Senior Machine Learning Engineer at Netflix/i)).toBeVisible();
  });
  ...
  test('should display footer', async ({ page }) => {
    ...
    await expect(page.getByText('// Data Engineering Open Forum')).toBeVisible();
...
test.describe('2025 Talks Page', () => {
  test('should display event metadata', async ({ page }) => {
    ...
    // Check title
    await expect(page.getByText('Data Engineering Open Forum 2025')).toBeVisible();

    // Check description
    await expect(page.getByText(/second edition of Data Engineering Open Forum/i)).toBeVisible();
    ...
    // Check talk count
    await expect(page.getByText('// 12 talks')).toBeVisible();
  });
  ...
  test('should display talk cards', async ({ page }) => {
    ...
    // Check that talk cards are rendered
    await expect(page.getByText(/How Netflix built a Real-Time Distributed Graph/i)).toBeVisible();
    await expect(page.getByText(/Apache Spark™ 4.0 for Data Engineering/i)).toBeVisible();
  });

  test('should display "Recording not available" for talks without YouTube links', async ({ page }) => {
    await page.goto('/past/2025');

    // Check that at least one "not available" message appears (there are multiple)
    await expect(page.getByText('// Recording not available').first()).toBeVisible();
  });
  ...
  test('should display Lightning Talks', async ({ page }) => {
    ...
    await expect(page.getByText(/Lightning Talks: Speed up your pipelines/i)).toBeVisible();
    await expect(page.getByText(/Lightning Talks: Do Work that Matters/i)).toBeVisible();
  });
});
