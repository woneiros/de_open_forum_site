...
export function MuseumAgendaPrototype() {
  ...
  const getSortedSpeakers = (session: Session) =>
    [...session.speakers].sort((left, right) => left.name.localeCompare(right.name));

  const getModerator = (session: Session) => session.moderator ?? null;

  const renderSpeakerList = (session: Session, align: "left" | "center" = "left") => {
    const sortedSpeakers = getSortedSpeakers(session);
    ...
    return (
      <div
        className={`mt-2 space-y-0.5 text-xs text-muted-foreground ${align === "center" ? "text-center" : "text-left"}`}
      >
        {sortedSpeakers.map((speaker) => (
          <p key={`${session.id}-${speaker.name}`}>
            {speaker.name}, {speaker.title} @ {speaker.company}
          </p>
        ))}
        {moderator ? (
          <p key={`${session.id}-${moderator.name}`}>
            {moderator.name}, {moderator.title} @ {moderator.company}
          </p>
        ) : null}
      </div>
    );
        ...
        {timeSlots.map((slot) => (
          ...
          >
            ...
            {slot.key === "07:30" ? (
              ...
              (() => {
                const slotSessions = sessions.filter((item) => item.start === slot.key);
                const spanningSession = slotSessions.find((item) => item.spansAllHalls);

                if (spanningSession) {
                  const isSelected = activeSession?.id === spanningSession.id;
                  return (
                    <div className="col-span-3 flex bg-black/10 p-1">
                      <button
                        ...
                        onClick={(event) => {
                          ...
                          setActiveSession(spanningSession);
                        }}
                        className={`flex h-full w-full cursor-pointer flex-col items-center justify-center rounded border border-accent/15 px-3 py-4 text-center transition-all ${
                          isSelected
                      ...
                      >
                        <p className="text-sm font-semibold leading-tight underline decoration-accent/70 underline-offset-2">
                          {spanningSession.title}
                        </p>
                        <p className="mt-2 font-mono text-[11px] tracking-[0.18em] text-accent/80">
                          @ {halls[spanningSession.hall].name}
                        </p>
                        {renderSpeakerList(spanningSession, "center")}
                        <p className="mt-2 text-xs text-accent">{getSessionTimeRange(spanningSession)}</p>
                      </button>
                    </div>
                  );
                }

                return hallOrder.map((hallId) => {
                  ...
                  >
                    {cellSessions.length === 1 ? (
                      ...
                      >
                        ...
                        {renderSpeakerList(cellSessions[0])}
              ...
              <div className="absolute inset-x-0 top-[8vh] px-4">
                ...
                >
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    {activeSession.speakers.length > 0 || activeSession.moderator ? (
                      <>
                        {activeSession.speakers.length > 0 ? (
                          <>
                            <p>
                              <span className="font-mono text-accent">SPEAKERS</span>
                            </p>
                            <div className="space-y-3">
                              {getSortedSpeakers(activeSession).map((speaker) => (
                                <div
                        ...
                        {activeSession.moderator ? (
                          <>
                            <p>
                              <span className="font-mono text-accent">MODERATOR</span>
                            </p>
                            <div className="space-y-3">
                              <div
                                key={`${activeSession.id}-${activeSession.moderator.name}`}
                                className="border-l-2 border-accent/30 pl-3 py-2"
                              >
                                <div className="flex items-start gap-3">
                                  {activeSession.moderator.avatarUrl ? (
                                    <img
                                      src={activeSession.moderator.avatarUrl}
                                      alt={`${activeSession.moderator.name} avatar`}
                                      className="h-20 w-20 rounded-full border border-accent/30 object-cover"
                                    />
                                  ) : (
                                    <div className="h-20 w-20 rounded-full border border-accent/30 bg-black/20" />
                                  ...
                                  <div className="min-w-0 flex-1">
                                    <div>
                                      <div className="flex items-center gap-1">
                                        <p className="min-w-0 font-semibold text-primary-foreground">
                                          {activeSession.moderator.name}
                                        </p>
                                        <a
                                          href={activeSession.moderator.linkedin}
                                          target="_blank"
                                          rel="noopener noreferrer"
                                          className="inline-flex items-center text-muted-foreground hover:text-accent"
                                          aria-label={`${activeSession.moderator.name} on LinkedIn`}
                                          title={`${activeSession.moderator.name} on LinkedIn`}
                                        >
                                          <svg
                                            viewBox="0 0 24 24"
                                            aria-hidden="true"
                                      ...
                                      <p className="font-mono text-sm text-muted-foreground">
                                        {activeSession.moderator.title} @ {activeSession.moderator.company}
                                      </p>
                                      <SpeakerBio bio={activeSession.moderator.bio} />
                                    </div>
                                  </div>
                                </div>
                              </div>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      {activeSession.sessionType !== "None" ? (
                        <div
                          className={`inline-block rounded border px-2 py-1 text-xs font-semibold ${sessionTypeStyle[activeSession.sessionType]}`}
                        >
